...
def prep_data(datadir, ratings_file, numratings=10000):
    ...
    ratings_file = '%s/%s' % (datadir, ratings_file)
    ...
    if numratings < nratings:
        sample=np.random.permutation(np.arange(nratings))[:numratings-1]
        ratings = ratings.ix[:numratings,:]
